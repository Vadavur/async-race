...
export class PopupRegisterForm extends BaseComponent {
  ...
  constructor(name: string) {
    ...
    this.inputsAttributes =
      REGISTER_FORM_INPUTS_ATTRIBUTES as InputAttributesInterface[];
  ...
  private setPopupInputs() {
    this.inputsAttributes.forEach((input) => {
      ...
      if (input.instance.element.getAttribute('type') === 'text') {
        input.instance.element.oninvalid = (event) => {
          const inputElement = event.target as HTMLInputElement;
          inputElement.setCustomValidity(
            inputElement.getAttribute('onInvalidValueText') as string
          );
        };
      }
